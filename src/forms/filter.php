...
$res = $db->query("
    SELECT 
      ...
      SUM(TOTAL) as Total,
      CAST(((SUM(RATE) * REELWEIGHT) + (SUM(RATE) * REELWEIGHT * (CAST(SUM(CGST) + SUM(SGST) + SUM(IGST) AS DECIMAL) / 100.0))) AS DECIMAL) / REELWEIGHT AS ACTUALPRICE,
      (SUM(TOTAL)/REELWEIGHT) as BUYINGPRICE,
      ROUND(((CAST(((SUM(RATE) * REELWEIGHT) + (SUM(RATE) * REELWEIGHT * (CAST(SUM(CGST) + SUM(SGST) + SUM(IGST) AS DECIMAL) / 100.0))) AS DECIMAL) / REELWEIGHT) - (SUM(TOTAL)/REELWEIGHT)), 2) as TaxSavings,
      ROUND(((CAST(((SUM(RATE) * REELWEIGHT) + (SUM(RATE) * REELWEIGHT * (CAST(SUM(CGST) + SUM(SGST) + SUM(IGST) AS DECIMAL) / 100.0))) AS DECIMAL) / REELWEIGHT) - (SUM(TOTAL)/REELWEIGHT)) * REELWEIGHT, 2) as Totalsavings
    FROM 
      $tablename 
    WHERE 
