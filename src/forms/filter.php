...
$res = $db->query("
    SELECT 
      ...
      ROUND(((CAST(((SUM(RATE) * REELWEIGHT) + (SUM(RATE) * REELWEIGHT * (CAST(SUM(CGST) + SUM(SGST) + SUM(IGST) AS REAL) / 100.0))) AS REAL) / REELWEIGHT) - (SUM(TOTAL)/REELWEIGHT)) * REELWEIGHT, 2) as Totalsav
    FROM 
      $tablename 
    WHERE 
